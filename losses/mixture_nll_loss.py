    ...
    def __init__(self,
        ...
        if isinstance(component_distribution, str):
            ...
            self.nll_loss = nn.ModuleList([loss_dict[dist](eps=eps, reduction='none')
                                           for dist in component_distribution]) # nn.ModuleList(LaplaceNLLLoss, LaplaceNLLLoss)
...
# if __name__ == '__main__':
    # multi_traj_propose = torch.randn(3, 2, 1, 2)
    # gt = torch.randn(3, 1, 2)
    # proposal_pi = torch.randn(3, 2, 1, 2)
    # cls_loss_region = MixtureNLLLoss(component_distribution=['laplace'] * output_dim + ['von_mises'] * output_head,
    #                reduction='none')
    # cls_loss_region = cls_loss_region(pred=multi_traj_propose[:, :, -1:].detach(),
    #                                        target=gt[:, -1:, :self.output_dim + self.output_head],
    #                                        prob=proposal_pi,
    #                                        mask=reg_mask[:, -1:]) * ((cls_mask) & (pred['num_equal'] >= 6))
