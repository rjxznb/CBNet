...
class QCNetEncoder(nn.Module):
    ...
    def forward(self, data: HeteroData) -> Dict[str, torch.Tensor]:
        map_enc = self.map_encoder(data) # 通过原始数据获得融入了map之后的编码
        agent_enc = self.agent_encoder(data, map_enc) # 通过原始数据和融入map之后的变化获得融入agents_interaction之后的编码；
        return {**map_enc, **agent_enc} # 返回这两个编码在decoder还要再次融入二者进行循环解码；
