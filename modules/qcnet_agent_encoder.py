...
class QCNetAgentEncoder(nn.Module):
    ...
    def forward(self,
        ...
        motion_vector_a = torch.cat([pos_a.new_zeros(data['agent']['num_nodes'], 1, self.input_dim),
                                     pos_a[:, 1:] - pos_a[:, :-1]], dim=1) # 把当前时间步的位置坐标设为0，然后之前时间步的坐标转换为向量模式，也就是差值；如果想改为agent-centric，可以在这里修改；
        ...
        if self.dataset == 'argoverse_v2': # 对相对坐标和相对速度转换为极坐标，每一块由两个变量组成，第一个是长度，第二个是角度；
            x_a = torch.stack(
            ...
            raise ValueError('{} is not a valid dataset'.format(self.dataset))
        x_a = self.x_a_emb(continuous_inputs=x_a.view(-1, x_a.size(-1)), categorical_embs=categorical_embs) # 对应全部agent的特征矩阵；
        ...
        rel_pos_t = pos_t[edge_index_t[0]] - pos_t[edge_index_t[1]] # 将全局绝对坐标转换为相对坐标
        ...
        return {'x_a': x_a} # shape(agents, history_time, hidden_dim)
