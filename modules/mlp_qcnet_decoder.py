...
class QCNetDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        # region_mlp区域预测头：
        region_mlp = torch.load('./region.pkl', map_location='cpu')
        self.region = MLPLayer(128, 128, 6)
        self.region.load_state_dict(region_mlp)
        self.region.requires_grad_(False)
        self.mode_emb = nn.Embedding(num_modes, hidden_dim) # 这里表示最初输入decoder的建议轨迹的模态数量，也就是随机生成的编码；
        ...
        self.m2m_refine_attn_layer = AttentionLayer(hidden_dim=hidden_dim, num_heads=num_heads, head_dim=head_dim,
                                                    dropout=dropout, bipartite=False, has_pos_emb=False)
        self.to_loc_propose_pos = nn.ModuleList([MLPLayer(input_dim=hidden_dim, hidden_dim=hidden_dim,
    ...
    def forward(self,
        ...
        x_a_refine = scene_enc['x_a'][:, -1].repeat(self.num_modes, 1) # 为了在refine的时候6个模态能用；
        ...
        logging.info(f"edge_index_t2m_shape{edge_index_t2m.shape}, refine{edge_index_t2m_refine.shape}")
        logging.info(f"edge_index_t2m:{edge_index_t2m}, refine{edge_index_t2m_refine}")
