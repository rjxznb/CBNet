...
class QCNetMapEncoder(nn.Module):
    ...
    def forward(self, data: HeteroData) -> Dict[str, torch.Tensor]:
        ...
        rel_pos_pt2pl = pos_pt[edge_index_pt2pl[0]] - pos_pl[edge_index_pt2pl[1]] # 计算每个waypoints和其所在pl的中心线起始位置求相对坐标距离；
        ...
        if self.input_dim == 2:
            r_pt2pl = torch.stack(
                [torch.norm(rel_pos_pt2pl[:, :2], p=2, dim=-1),
                 ...
                 rel_orient_pt2pl], dim=-1) # 转换为极坐标和相对角度；
        elif self.input_dim == 3:
            r_pt2pl = torch.stack(
                [torch.norm(rel_pos_pt2pl[:, :2], p=2, dim=-1),
                 angle_between_2d_vectors(ctr_vector=orient_vector_pl[edge_index_pt2pl[1]],
            ...
            raise ValueError('{} is not a valid dimension'.format(self.input_dim))
        r_pt2pl = self.r_pt2pl_emb(continuous_inputs=r_pt2pl, categorical_embs=None) # 转换为傅里叶编码；
