...
class complexity_predict(pl.LightningModule):
    def __init__(self, input_dim = 128, hidden_dim = 128, output_dim = 2): # 输出的区域数
        super().__init__()
        ...
        self.complexity = MLPLayer(input_dim, hidden_dim, output_dim)
        self.complexity_loss_function = nn.CrossEntropyLoss( )
    ...
    def forward(self, data: HeteroData):
        ...
        pred_complexity = self.complexity(m1) # shape(agents, 128)
        return pred_complexity.to(torch.float)
    ...
    def training_step(self,
        ...
        pred = self(data) # shape(agents, 2)
        pred = pred[t] # shape(scored_agents, 2)
        complexity_loss = self.complexity_loss_function(pred, data['complexity'].to(torch.long)[t]) # data['complexity'] shape(agents, 1) --> shape(scored_agents, )
        self.log('train_complexity_loss', complexity_loss, prog_bar=True, on_step=True, on_epoch=True, batch_size=1)
    ...
    def validation_step(self,
        ...
        complexity_loss = self.complexity_loss_function(pred, data['complexity'].squeeze(-1).to(torch.long)[t]) # data['complexity'] shape(agents, 1) --> shape(scored_agents, )
        self.log('val_complexity_loss', complexity_loss, prog_bar=True, on_step=True, on_epoch=True, batch_size=1)
...
if __name__ == '__main__':
    complexity_module = complexity_predict()
    datamodule = ArgoverseV2DataModule(train_batch_size=2, val_batch_size=2, num_workers=4, pin_memory=False)
    model_checkpoint = ModelCheckpoint(dirpath="/space/renjx/checkpoints/complexity_predicting", monitor='val_complexity_loss', save_top_k=5, mode='min')
    ...
    trainer.fit(complexity_module, datamodule) # 如果传入的第二个参数train_dataloader是pl的dataloader那么其他dataloader参数就不能再传啦；
